function [u,p,ta]=PulsatileFlow(r,ru,mu,freq,p0,pn,phi,timestep,grid,T_num)
...
h=r/rxl; 
nw=length(pn);
omega=2*pi*freq; u=zeros(grid,1); p=zeros(1,timestep); ta=zeros(1,timestep); zt=zeros(1,timestep+1); 
alpha=r*sqrt(omega*ru/mu); 
% kapa=alpha*i^(1.5)/r;
% to make the waveform
snw=nw*(nw+1)/2; 
% alpha=alpha*sqrt(snw);
for k=1:timestep
    t=T_num;
    for l=1:nw
...
%             u(m+ofst,1)=-p0*((ri*h)^2-r^2)/4/mu+real(i/ru/omega/snw*(1- CBJ0/CJA)*zt(k));
%          end
%     end
 ...
 for m=-rxl:rxl;
     n=0; ri=sqrt(m^2+n^2);
        for l = 1:length(pn)
            kapa=alpha*(l^(0.5))*i^(1.5)/r;
             if ri*h<r
                CJA = besselj(0,kapa*r);
                CBJ0=besselj(0,kapa*h*ri);
                u(m+ofst,1)=u(m+ofst,1)+real((i*pn(l))/ru/omega/l*(1- CBJ0/CJA)*exp(i*(omega*t*l-phi(l))));
             end
        end
            u(m+ofst,1)=(-p0*((ri*h)^2-r^2)/4/mu)+u(m+ofst,1);
 end    
